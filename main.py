...
tree = ET.parse(current_working_directory + "/BBM211219230002-000079.XML")
root = tree.getroot()
...
def my_fhir_library_implementation():
    ...
    patient = ptn.PatientResource("0", patient_id, patient_sex,
                                  patient_birthdate)
    ...
    for sample in root.iter(
        ...
        sample_quantity = float((sample.find(
            r"{http://www.bbmri.cz/schemas/biobank/data}availableSamplesNo")
                                 .text))
...
def fhir_client_implementation():
    ...
    for sample in root.iter(
        ...
        fhir_sample_identifier_code.system = \
            "http://terminology.hl7.org/CodeSystem/v2-0203"
        fhir_sample_identifier_code.userSelected = True

        fhir_sample_identifier_codeable_concept = \
            fhirclient_codeable_concept.CodeableConcept()
        ...
        fhir_diagnosis_codeable_concept = \
            fhirclient_codeable_concept.CodeableConcept()
    ...
    fhir_entry_patient_request.url = "patient/" + fhir_patient.id
    ...
    fhir_entry_patient.fullUrl = "https://example.com/patient/" + \
                                 fhir_patient.id
