...
import fhirclient.models.condition as fhirclient_condition
import fhirclient.models.fhirdate as fhirclient_date
from fhir_library.patient import PatientResource
from fhir_library._Constants import META_PROFILE_URL, ICD_CODING_SYSTEM
...
class ConditionResource:
    ...
    def _convert_to_FHIR(self):
        ...
        """
        condition = fhirclient_condition.Condition()

        condition.meta = fhirclient_meta.Meta()
        condition.meta.profile = [META_PROFILE_URL["Condition"]]

        coding = Helper.create_coding(self._conditionCode, ICD_CODING_SYSTEM,
                                      user_selected=False)
        codeable_concept = Helper.create_codeable_concept([coding])

        fhir_date = fhirclient_date.FHIRDate()
