...
from fhir_library.custodian import Custodian
...
from fhir_library._Constants import SPECIMEN_TYPE, META_PROFILE_URL, \
    BODY_SITE_SYSTEM, \
    SPECIMEN_QUANTITY_SYSTEM, SPECIMEN_TYPE_SYSTEM, IDENTIFIER_TYPE_CODES
...
class SpecimenResource(object):
    """
    This class represents all the necessary information needed for a sample to
    be used for analysis.
    """

    def __init__(self, specimen_id:str, identifier: str, specimen_material_code: str,
        ...
        if identifier_type not in IDENTIFIER_TYPE_CODES:
            raise Exception(
                "{} in identifier_type is not a correct type!"
                "identifier type has to be one of the following".format(
                    identifier_type) +
                " ".join(
                    ["{}"] * len(IDENTIFIER_TYPE_CODES)).format(
                    *IDENTIFIER_TYPE_CODES))
    ...
    def _convert_to_FHIR(self):
        ...
        """
        FHIR_specimen = fhirclient_specimen.Specimen()

        FHIR_specimen.id = self._specimenId

        FHIR_specimen.identifier = [
            Helper.create_identifier(self._identifier,
                                     identifier_type=self._identifierType)]
        FHIR_specimen.meta = fhirclient_meta.Meta()
        FHIR_specimen.meta.profile = [META_PROFILE_URL["Specimen"]]

        fhir_specimen_type_coding = Helper.create_coding(
            self._specimenMaterialCode,
            SPECIMEN_TYPE_SYSTEM, user_selected=False)
        fhir_specimen_type_codeable_concept = Helper.create_codeable_concept(
            [fhir_specimen_type_coding])

        collection = fhirclient_specimen.SpecimenCollection()

        fhir_collected_time = fhirclient_date.FHIRDate()
        ...
        if self._bodySiteCollectionCode is not None:
            fhir_body_site_collection_coding = Helper.create_coding(
                self._bodySiteCollectionCode, BODY_SITE_SYSTEM,
                user_selected=False)
            fhir_body_site_collection_codeable_concept = \
                Helper.create_codeable_concept(
        ...
        subject_reference = Helper.create_fhir_reference(
            "Patient/" + self._subject.patientId)

        container = fhirclient_specimen.SpecimenContainer()
        quantity = fhirclient_quantity.Quantity()
        quantity.value = self._quantity
        quantity.unit = self._quantityUnit
        quantity.code = self._quantityUnitCode
        quantity.system = SPECIMEN_QUANTITY_SYSTEM
