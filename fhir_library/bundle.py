...
from fhir_library._Constants import BUNDLE_REQUEST_METHOD
from fhir_library._Constants import BUNDLE_TYPES
...
class Entry:
    ...
    def __init__(self, resource: Union[PatientResource, SpecimenResource,
        ...
        if request_method not in BUNDLE_REQUEST_METHOD:
            raise TypeError(
                "{} in request_method is not correct method! "
                "request_method has to be one of the following".format(
                    request_method) +
                " ".join(["{}"] * len(BUNDLE_REQUEST_METHOD)).format(
                    *BUNDLE_REQUEST_METHOD))
    ...
    def _convert_to_FHIR(self):
        ...
        """
        FHIREntry = fhirclient_bundle.BundleEntry()

        entry_request = fhirclient_bundle.BundleEntryRequest()
...
class Bundle:
    ...
    def _convert_to_FHIR(self):
        """
         ...
         self.bundleJson()
        """
        FHIRBundle = fhirclient_bundle.Bundle()
