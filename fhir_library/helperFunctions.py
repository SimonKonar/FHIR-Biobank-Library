...
import fhirclient.models.codeableconcept as fhirclient_codeableconcept
import fhirclient.models.identifier as fhirclient_identifier
import fhirclient.models.fhirreference as fhirclient_FHIRreference

from fhir_library import _Constants
...
class Helper:
    ...
    def create_coding(code: str, system: str = None, version: str = None,
        ...
        coding = fhirclient_coding.Coding()
        coding.code = code
    ...
    def create_codeable_concept(
            list_of_codings: List[fhirclient_coding.Coding],
            text: str = None):
        """
        ...
        for code in list_of_codings:
            if not isinstance(code, fhirclient_coding.Coding):
                raise TypeError(
        ...
        codeable_concept = fhirclient_codeableconcept.CodeableConcept()
    ...
    def create_identifier(value: str, use: str = "usual",
        ...
        if use not in _Constants.IDENTIFIER_USE:
            raise Exception(
                "{} in use is not a correct type!"
                "use type has to be one of the following".format(
                    use) +
                " ".join(
                    ["{}"] * len(_Constants.IDENTIFIER_USE)).format(
                    *_Constants.IDENTIFIER_USE))
        if not isinstance(identifier_type, str):
            raise TypeError("identifier_type has to be a string!")

        if identifier_type not in _Constants.IDENTIFIER_TYPE_CODES:
            raise Exception(
                "{} in identifier_type is not a correct type!"
                "identifier type has to be one of the following".format(
                    identifier_type) +
                " ".join(
                    ["{}"] * len(_Constants.IDENTIFIER_TYPE_CODES)).format(
                    *_Constants.IDENTIFIER_TYPE_CODES))

        identifier = fhirclient_identifier.Identifier()

        type_code = Helper.create_coding(identifier_type,
                                         system=_Constants.IDENTIFIER_CODES_SYSTEM)
    ...
    def create_fhir_reference(reference: str):
        ...
        fhir_reference = fhirclient_FHIRreference.FHIRReference()
