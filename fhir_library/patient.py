...
from fhir_library._Constants import PATIENT_GENDER, META_PROFILE_URL, \
...
class PatientResource:
    ...
    def __init__(self, patient_id: str, identifier: str,
                 gender: str = "unknown", birth_date: date = None,
                 deceased_boolean: bool = False,
                 deceased_datetime: date = None,
        ...
        if gender not in PATIENT_GENDER:
            raise ValueError(
                "{} in variable gender is not correct "
                "code for gender."
                "code has to be one of the following".format(
                    gender) +
                " ".join(["{}"] * len(PATIENT_GENDER)).format(*PATIENT_GENDER))
        ...
        if deceased_datetime is not None and deceased_datetime > date.today():
            raise ValueError(
    ...
    def _create_patient_link(self, patients):
        ...
        for patient in patients:
            ...
            patient_link = fhirclient_patient.PatientLink()
            patient_link.other = Helper.create_fhir_reference(
    ...
    def _convert_to_FHIR(self):
        ...
        if self._FHIR_Patient is None:
            ...
            FHIR_Patient.meta = fhirclient_meta.Meta()
            FHIR_Patient.meta.profile = [META_PROFILE_URL["Patient"]]
            identifier = [Helper.create_identifier(self._identifier,
                                                   identifier_type=
            ...
            if self._birthDate is not None:
                fhir_birth_date = fhirclient_date.FHIRDate()
            ...
            if self._deceasedDatetime is not None:
                fhir_deceased_datetime = fhirclient_date.FHIRDate()
